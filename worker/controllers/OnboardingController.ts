...
export class OnboardingController {
  ...
  async handle(request: Request): Promise<Response> {
    ...
    if (path === '/api/onboarding/join' || path === '/api/onboarding/invitation') {
      ...
      const invite = await db.prepare("SELECT i.organizationId, i.email, i.expiresAt, o.name, EXISTS (SELECT 1 FROM member m WHERE m.organizationId = i.organizationId AND m.userId = i.creatorId AND m.role IN ('owner', 'admin')) AS authorized FROM onboardingInvite i JOIN organization o ON o.id = i.organizationId WHERE i.tokenHash = ?")
        .bind(hash).first<{ organizationId: string; email: string | null; expiresAt: number; name: string; authorized: number }>()
      if (!invite) return Response.json({ message: 'Convite indisponível. É necessário um novo convite.', code: 'INVITATION_NOT_FOUND' }, { status: 403 })
      if (invite.expiresAt <= Date.now()) return Response.json({ message: 'O convite expirou. É necessário um novo convite.', code: 'INVITATION_EXPIRED' }, { status: 403 })
      if (!invite.authorized) return Response.json({ message: 'O convite deixou de estar ativo. É necessário um novo convite.', code: 'INVITATION_REVOKED' }, { status: 403 })
      if (invite.email && invite.email !== session.user.email.toLowerCase()) return Response.json({ message: 'A conta com sessão iniciada não corresponde ao email do convite. É necessário utilizar o email que recebeu o convite.', code: 'INVITATION_ACCOUNT_MISMATCH', recipientEmail: invite.email }, { status: 403 })
