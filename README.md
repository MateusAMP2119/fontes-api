...
Remote development inherits the deployed Worker's secrets. If overriding credentials
with `.dev.vars`, use `.dev.vars.example`.
For a local API development session, set the frontend's `VITE_API_URL` to
`http://localhost:8788` and run `fontes-app` on port 5173. Use email registration
and password login locally. Google sign-in is available only with the production
API; no local OAuth proxy or shared proxy secret is required.
