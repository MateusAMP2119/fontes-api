...
test('production auth config supports OTP migration, password login, reset and single-use tokens',async()=>{
 ...
 const changed=await f.call('/set-password',{currentPassword:'second-password-456',newPassword:'third-password-789',revokeOtherSessions:true},cookies(loggedIn))
...
test('set-password requires current password, with empty input only for first setup',async()=>{
 ...
 assert.equal((await f.call('/set-password',{currentPassword:'',newPassword:'valid-password-123'})).status,401)
 assert.equal((await f.call('/set-password',{newPassword:'valid-password-123'},cookie)).status,400)
 assert.equal((await f.call('/set-password',{currentPassword:'wrong',newPassword:'valid-password-123'},cookie)).status,400)
 assert.equal((await f.call('/set-password',{currentPassword:'',newPassword:'short'},cookie)).status,400)
 assert.equal((await f.call('/set-password',{currentPassword:'',newPassword:'valid-password-123'},cookie)).status,200)
 assert.equal((await f.call('/set-password',{currentPassword:'',newPassword:'replacement-password'},cookie)).status,400)
 assert.equal((await f.call('/set-password',{currentPassword:'wrong',newPassword:'replacement-password'},cookie)).status,400)
 assert.equal((await f.call('/set-password',{currentPassword:'valid-password-123',newPassword:'replacement-password'},cookie)).status,200)
 ...
 assert.ok(schema.paths['/api/auth/set-password'])
