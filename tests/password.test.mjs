...
test('production auth config supports OTP migration, password login, reset and single-use tokens',async()=>{
 ...
 const changed=await f.call('/set-password',{password:'second-password-456','new-password':'third-password-789',revokeOtherSessions:true},cookies(loggedIn))
...
test('set-password requires current password, with empty input only for first setup',async()=>{
 ...
 assert.equal((await f.call('/set-password',{password:'','new-password':'valid-password-123'},cookie)).status,200)
 ...
 assert.equal((await f.call('/set-password',{password:'valid-password-123','new-password':'replacement-password'},cookie)).status,200)
